...
def hook_init(fm):
    ...
    for disk in "abcdefgh":
        fm.execute_console("map {key}{0} chain shell udiskie-mount /dev/sd{1}".format(
            disk.upper(), disk, key=MOUNT_KEY))
        fm.execute_console("map {key}{0} chain cd; chain shell udiskie-umount /dev/sd{1}".format(
            disk.upper(), disk, key=UMOUNT_KEY))
        for part in "123456789":
            fm.execute_console(
                "map {key}{0}{1} chain shell udiskie-mount /dev/sd{0}{1}".format(
                    disk, part, key=MOUNT_KEY)
            )
            fm.execute_console("map {key}{0}{1} chain cd; shell udiskie-umount /dev/sd{0}{1}".format(
